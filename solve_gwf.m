...
function P = solve_gwf(coef,F,g)
    ...
    F(1,:)= F(1,:)+(coef(1,2:K-1)+coef(2,2:K-1))/2.*g(1,2:K-1)*(K-1)^2;
    F(K-2,:)= F(K-2,:)+(coef(K-1,2:K-1)+coef(K,2:K-1))/2.*g(K,2:K-1)*(K-1)^2;
    F(:,1)= F(:,1)+(coef(2:K-1,1)+coef(2:K-1,2))/2.*g(2:K-1,1)*(K-1)^2;
    F(:,K-2)= F(:,K-2)+(coef(2:K-1,K-1)+coef(2:K-1,K))/2.*g(2:K-1,K)*(K-1)^2;
	d = cell(K-2,K-2);
